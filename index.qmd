...
[This chapter](https://www.fuzzingbook.org/html/Reducer.html) in the fuzzing
book introduces us to the idea of *Delta Debugging* for pinpointing faulty areas
of code. By procedurally trimming or removing parts of an input, we can
determine the exact point at which an input fails and then create a conjecture
concerning why the said part of the input failed.

One example of delta debugging is by systematically dividing a string input to a
function according to an algorithm that resembles binary search. We start by
chunking the string in half and running each half through the test function. If
only one side fails, we keep repeating this process on the half that broke the
function in order to find the minimal failing input. From such a small input, we
can then determine exactly why our code fails at this part.

Another, more complex, example is to apply a grammar to intelligently split an
input up. By this method, you are no longer splitting the input string up by
random characters, you are splitting it up by tokens determined from the
grammar.

This method of debugging is extensible to a wide variety of different inputs,
although more effective on some than others.

#### Action Items & Reflection

The `chasten` and `cellveyor` teams will need to keep the principles of delta
debugging in mind when coming across issues. We will most certainly have issues
in our programs, and it will be important to have this method of debugging at
our disposal. While it may not make the most sense to devote too much time to
debugging by this method (mostly because of the smaller scope of our project),
we can still benefit from knowing this method of debugging.

* **Start doing:** Keeping in mind the concepts of delta debugging in order to
  better apply its principles.
* **Stop doing:** N/A
* **Keep doing:** Having our minds open to new debugging concepts.

# Week 13

## SE10: Documentation

[This chapter](https://abseil.io/resources/swe-book/html/ch10.html) in the
Software Engineering Book at Google tells us about the value of
**documentation**. The benefits of documentation are not immediately felt, which
is what makes writing it so difficult for the begrudging developer. In the
effort of "optimizing for the reader," there are many important principles to
take into account.

* Documentation is like code: Documentation should be managed and edited just
  like code is. This means that documentation is subject to issues, pull
  request, feature requests, and the most important of all, having bugs.
* Know your audience: It's one thing to write for yourself, but one must
  consider the audience that will be reading the documentation that they have
  written. This includes writing for *seekers*, people who know exactly what
  they want, and *stumblers*, people who are not quite sure where to look. To
  cater to both means having all levels of technical depth, from beginner to
  expert.
* Acknowledge that there are different types of documentation: Depending on the
  documentation type, the author will need to distinguish what kind of
  documentation type they are creating. This will determine how in-depth they go
  with the information, and it will determine how much effort they put in to
  making it accessible.
* Have someone review your documentation: Similarly to how code is reviewed, it
  is worth having someone review your documentation in order to verify that it
  is technically correct, legible, and consistent.
* Deprecate documents, when necessary: It is important to know when to deprecate
  documents. This may involve pointing the reader to a new document, or it may
  involve completely writing the document off as obsolete.

Above all, reminding oneself that documentation is crucial to scaling a team or
company will encourage good authorship. Documentation cannot be written alone,
and a team must work together to write excellent documents to outlive their own
goals and time in that team.

#### Summary

This chapter struck me as very important information. I have completely changed
my opinions about documentation after being in this class. I have even found
myself writing documentation in the code that I work on outside of class. I
think that between the `chasten` and `cellveyor` teams, we are doing an
excellent job of documenting our technical work. The only part where where we
fall short is in the creation of our wiki. That mostly falls under one or two
people.

#### Action Items & Reflection

* **Start doing:** Investing time into beefing up our wiki.
* **Stop doing:** N/A
* **Keep doing:** Writing well-documented code.

## DB1: Introduction to Debugging

#### Summary

[This chapter](https://www.debuggingbook.org/html/Intro_Debugging.html) in the
debugging book walks us through how to effectively debug code, at the
introductory level. We are first told how, when encountering a bug, we can
easily take reactive steps that do not effectively pinpoint the issue. Zeller
refers to these as steps from the "devil's guide."

Zeller provides three good patterns that apply to debugging in general:

1. Understand the code
2. Fix the problem, not the symptom
3. Proceed systematically

When defects are not pinpointed, they propagate into failure. Often, a software
engineering will only look at the failure rather than piecing apart the steps to
locate the defect. Zeller encourages the reader to apply the scientific method,
in which one conjures and tests a hypothesis.

Through rigorous, repeated testing, one can test their hypothesis to assert its
validity. After this point, the developer may be able to pinpoint the issue at
hand and take steps to fix it. As an alternative to many of these technical
steps, Zeller promotes the idea of "rubber ducking," which is when you explain
the problem to a colleague. This trick will often surprise the individual trying
to debug their program, because, more often than not, the nonsensical bit will
jump out while they are trying to explain their issue!

#### Action Items & Reflection

This chapter was not extremely technical, with which I have no issue. I felt
that many of the points in this article were trivial, however. Zeller could have
omitted them by the fact that they are mostly common knowledge, such as the idea
of forming a hypothesis. I appreciated the thorough introduction to debugging,
however. This may prove to be more insightful than many of the topics from the
fuzzing book, simply because of the pragmatism seen here.

* **Start doing:** Being willing to form hypotheses when our code fails.
* **Stop doing:** N/A
* **Keep doing:** N/A
...
[This chapter](https://abseil.io/resources/swe-book/html/ch11.html) of the
Software Engineering Book at Google discusses the value of testing. Testing
frees the developer to make changes with confidence, as they can quickly
diagnose and see which functionality is broken by their changes.
...
* **Less debugging**: Given that tested code has fewer defects, there is naturally
  ...
  frees the developer to focus on other things.
...
The `chasten` and `cellveyor` teams have already exercised the use of testing
extensively. I have even seen accounts of individuals putting pressure on their
teammates to add testing to a new feature. The culture that we have concerning
testing is, in my opinion, very healthy. While not everyone may seek to create
tests or need to create tests for every feature, the majority of our operations
involve the application of testing. From my own experience, adding test cases to
my `chasten` feature allowed me to feel confident subjecting it to review, which
allows me to move forward without looking back!

#### Action Items & Reflection

* **Start doing:** Ensuring each person is adding test cases to their new
  feature and that the test cases are well-explained.
* **Stop doing:** N/A
* **Keep doing:** Implementing testing in our tools and referencing test cases
  as a way to validate someone else's work.

## DB2: Tracing Executions

#### Summary

[This chapter](https://www.debuggingbook.org/html/Tracer.html) of the debugging
book shows us how we can trace our program and flag specific events for the
purpose of debugging. By using `sys.settrace()` in tandem with the
`StackInspector` class, we can track events that happen in code without having
to write out numerous calls to `print`. The author defines a class that prints
out each executed line of a program along with the entire scope. This prints out
all of the variables and their values for each step.

An extension of this is to trace the changes to a variable. This is done by
attaching onto each event to determine which of the variables in scope have
changed in value. This can be useful for tracing unintentional changes or for
tracing when a variable was changed to an undesired value.

In addition to dynamic tracers used for tracing the execution of dynamically
typed languages, we can also employ what are called *static tracers*. These can
be used for tracing the execution of compiled languages such as C. Additionally,
a developer can take advantage of the x86 architecture's feature of hardware
breakpoints, which are ways to pause the execution of a program and inspect the
memory.

Each language has a way to debug its code with some form of metaprogramming.
These approaches can relieve the developer of having to modify their code and
allows them to trace back to better pinpoint an issue.

#### Action Items & Reflection

I feel that the test suite of `chasten` would benefit from an improved way to
trace. I have found myself scratching my head when the test suite breaks without
providing a way to trace back to where a certain test broke. If we took the
extra hours to implement a way to trace where a program failed, we could enhance
our test suite *even more*.

* **Start doing:** Employing tracing methods of debugging as a part of our test
  suite.
* **Stop doing:** N/A
* **Keep doing:** N/A
...
[This chapter](https://abseil.io/resources/swe-book/html/ch12.html) in the
Software Engineering Book at Google discusses what unit testing is. Unit tests
are small tests that test a very specific functionality in a larger program. As
opposed to larger tests with a wide scope, unit tests demonstrate the smallest
"unit" of functionality, hence the name "unit test."

An antipattern while creating unit tests is to create *brittle tests*. Brittle
tests are tests that can be easily broken by a few minor changes. These kinds of
tests rely on multiple hard-coded attributes of a program, such as a data type
or namespace.

This chapter provides an example of a developer trying to test a
public API. While the developer may be tempted to reference the API's
functionality by directly interfacing its private members, the better option
would be to test it from the same perspective that the end user would. The book
states that these kinds of tests "form explicit contracts: if such a test
breaks, it implies that an existing user of the system will also be broken."

One other example of a brittle test is a test that tests the interactions of a
functionality rather than the state it produces. These kinds of tests measure
how a system takes steps to complete an action instead of comparing the final
state of the application after it performs an operation. Once again, this is the
difference between tests that test the behind-the-scenes instead of testing the
more public facing side of an application. This chapter makes an argument for
testing a program in a method similar to how the user would see it.

The most important part of writing effective unit tests is being *clear* and
*concise*. The clarity required for writing detailed tests pays off when other
developers read over your code. The conciseness required for completing tests
pays off when a single test fails and one can immediately tell what has gone
wrong. Tests that are lacking in these two areas will be improperly named, test
more than one part of functionality, or not be immediately obvious in what they
are testing. These kinds of tests delay development and pose as a liability.

Unit tests are one of the most powerful tools that are at a software engineer's
disposal. Careful use of unit tests will yield incredible velocity with which a
team can develop.

#### Action Items & Reflection

This chapter goes into great depth on the different semantics that exist in how
tests are written. I felt confused on numerous accounts during this chapter
because of how specific the differences were between the kinds of tests. I wish
that I could resonate with this chapter more, because I believe that it could be
more helpful. I keep thinking of future endeavors I will engage in, most likely
in the context of scientific computing. I hope that I can incorporate these
kinds of software engineering practices into my workflow.

* **Start doing:** Keeping in mind *how* we are writing our tests and if the
  test case could be broken up into smaller bits or combined with similar logic.
* **Stop doing:** Writing super large integration tests that slow down our test
  suite.
* **Keep doing:** Writing tests whenever we ship code.

## DB3: Assertions

#### Summary

[This chapter](https://www.debuggingbook.org/html/Assertions.html) in the
Debugging Book discusses the use of assertions in verifying a function's
expected behavior. Assertions are used in debugging, and they are often disabled
when optimizations are turned on. Zeller explains how they function similarly in
C: they are disabled in production by a precompile flag.

In the context of a function, assertions can either be used to validate the
arguments to the function, or they can be used to validate the return of the
function. These are known as *preconditions* and *postconditions*. As an
example, we can apply this to a function that removes html markups:
...
The *postcondition* here will break `remove_html_markup` at runtime if it
detects that the function has failed. This not only ensures a valid input, but
it also demonstrates explicitly what the purpose of the function is.

Ultimately, assertions allow us to ensure a specified behavior by embedding them
directly into programming logic. They can be applied to important features such
as *memory allocators*, or they can be applied to simple functions that parse a
DateTime object from a string input.

#### Action Items & Reflection

I feel that we could benefit from embedding assertions into some of our code.
Regardless of it is in `chasten` or `cellveyor`, we are dealing with user input,
and that user input needs to be validated before we use it. By embedding
assertions into parts of our program logic, we could treat the arguments to a
program as if they already conformed to a specific standard. The only caveat to
this is that the program could fail unexpectedly on the user side, which could
provide a poor user experience. If the program failed on the user's side and the
user did not realize that it was the program's fault, they may spend hours
trying to figure out what they did wrong, or the user may reject using one of
our tools altogether.

* **Start doing:** Considering where it would be logical to embed assertions in
  `chasten` and `cellveyor`.
* **Stop doing:** N/A
* **Keep doing:** N/A
...
[This chapter](https://www.debuggingbook.org/html/StatisticalDebugger.html) in
the Debugging Book discusses statistical debugging. Statistical debugging can
correlate specific parts of a program with the likelihood of failure. For
example, when executing a program that performs an operation, if we find for 60%
of its bad runs that it breaks on line 17, and 40% of its bad runs break on line
18, we can logically conclude that between lines 17-18 there is an issue. From
this information, we can examine the inputs being used at those lines, the
functions being called, or the operations being performed. This approach is
known as *statistical debugging*.

Statistical debugging begins with a collector. The collector is responsible for
'collecting' each line of a function and testing whether or not the program
failed for all of those lines run. After multiple runs, we can see which lines
are unique to the failing runs, if any, and we can see which lines are unique to
the passing runs, if any.

To enhance our analysis, we can see which lines, on the basis of percentage, are
run *more* by failing or passing runs. Now, hot spots of code that are likely to
fail can be *localized*.

Software testing enthusiasts have figured out metrics for determining
*suspiciousness*, which is a way to quantify this likelihood of breaking a run.

Statistical debugging can give helpful insight into how a program fails over a
long, measured period of time. Although it is computationally expensive to
calculate suspicious areas of code, the metric can be helpful as a debugger's
last resort to pinpointing an issue.

#### Action Items & Reflection

This debugging technique is extremely interesting. So interesting that I may
have to employ it in my own work. The only problem is that I often create
programs that are not 'runnable,' per se. Most of the programs that I create
have an infinite loop and involve real-time user input. I will take it up in my
own time to explore statistical debugging for this kind of environment.

* **Start doing:** Exploring the ideas of statistical debugging.
* **Stop doing:** N/A
* **Keep doing:** N/A
